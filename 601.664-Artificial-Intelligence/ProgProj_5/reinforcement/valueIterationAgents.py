...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for _ in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                if self.mdp.isTerminal(state):
                    ...
                    curValue = [self.getQValue(state, a) for a in actions]
                    newValues[state] = max(curValue)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for newstate, prob in statesAndProbs:
          ...
          Q = prob * (reward + self.discount * self.values[newstate])
          Qvalue += Q
        return Qvalue
    ...
    def computeActionFromValues(self, state):
        ...
        best_policy, best_action = -9999999, None
        for a in actions:
            cur_policy = self.computeQValueFromValues(state, a)
            if cur_policy > best_policy:
                best_policy, best_action = cur_policy, a
        return best_action
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            if self.mdp.isTerminal(state):
                ...
                curValue = []
                for a in actions:
                    curValue.append(self.getQValue(state, a))
                bestValue = max(curValue)
