...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                n_vc =visited_corners.copy()
                #print(n_vc)
                if (nextx,nexty) in self.corners:
                    if (nextx,nexty) not in n_vc:
                        n_vc.add((nextx,nexty))
                successors.append((((nextx, nexty),set(n_vc)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    heu_list = [util.manhattanDistance(node, (x,y)) for node in non_visited]

    return max(heu_list) if heu_list else 0
...
def foodHeuristic(state, problem):
    ...
    return max(food_distance) if food_distance else 0
